...
function iniciar() {
	...
	if ( request.method === 'OPTIONS' ) {
		...
		else if(pathname == '/songs/audio1.mp3'){

			console.log('audio');
            fs.readFile('./songs/audio1.mp3', function (err, sortida) {
                response.writeHead(200, {
		...
		else if(pathname == '/songs/Arctic_Monkeys_Do_I_Wanna_Know.mp3'){
			
						console.log('audio');
						fs.readFile('./songs/Arctic_Monkeys_Do_I_Wanna_Know.mp3', function (err, sortida) {
							response.writeHead(200, {
								'Content-Type': 'audio/mpeg'
							});
